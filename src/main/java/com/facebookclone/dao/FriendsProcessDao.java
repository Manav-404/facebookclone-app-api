...
public interface FriendsProcessDao {
	...
	@Query(value = "Select * from friends where user_one_id=?1 or user_two_id=?1 and status=0 and user_action != ?1" ,nativeQuery = true)
	...
	public boolean deleteFriendRequest(long friendId , long currentUserId);
