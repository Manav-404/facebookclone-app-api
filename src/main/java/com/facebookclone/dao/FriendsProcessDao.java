...
public interface FriendsProcessDao {
	...
	@Query(value = "Select * from friends where user_one_id=?1 or user_two_id=?1 and status=0 and user_action !=?1 and status=0")
	public List<FriendsProcess> getToBeAcceptedList(long currentUserId);
	
	@Query(value = "Update friends set status=1 where user_one_id=?1 or user_two_id=?1 and user_action=?2")
	public FriendsProcess acceptFriend(long friendId , long currentUserId);
	
	@Query(value = "Delete from friends where user_one_id=?1 or user_two_id=?1 and status =0 and user_action=?2")
